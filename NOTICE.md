...
- 来源：**本项目自有素材**（自备原图，缩放后用于 App 图标与 App 内品牌标识）
- 许可：本项目自有 —— **不是**第三方图标库的图形，
  因此本节不列任何第三方署名

> 本节编号保持不变，以避免改动下文与 `README.md` 的交叉引用。
> 之所以明确写出来：早期版本确实用过第三方图标库，后来全部改为自有素材，
> 相应的署名已移除 —— 给一个**不再使用**的组件署名同样是错误的。
